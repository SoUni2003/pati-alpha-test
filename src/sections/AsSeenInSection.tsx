...
const AsSeenInSection = () => {
  return (
    <section className='border-y bg-sculptique-yellow py-[20px] overflow-hidden'>
      <div className='flex w-full flex-col items-center gap-6 text-center'>
        <h3 className='font-trirong font-medium text-lg lg:text-2xl text-black px-4'>As Seen In</h3>
          ...
          <div className='flex animate-infinite-scroll items-center gap-12 whitespace-nowrap pb-2 lg:gap-20 pr-12 lg:pr-20'>
            {LOGOS.map((src, index) => (
              <img
                ...
                className='h-5 lg:h-[30px] w-auto opacity-90 grayscale transition-all hover:grayscale-0'
                loading='lazy'
              />
            ))}
            {LOGOS.map((src, index) => (
